...
class GRU4RecPlus(SeqAbstractRecommender):
    ...
    def __init__(self, **kwds):
        ...
        self.n_sample = self.conf["n_sample"]
        self.sample_alpha = self.conf["sample_alpha"]
        self.epochs = self.conf["epochs"]
        self.bpr_reg = self.conf["bpr_reg"]

        if self.conf["hidden_act"] == "relu":
            self.hidden_act = tf.nn.relu
        elif self.conf["hidden_act"] == "tanh":
            self.hidden_act = tf.nn.tanh
        else:
            raise ValueError("There is not hidden_act named '%s'." % self.conf["hidden_act"])

        # final_act = leaky-relu
        if self.conf["final_act"] == "relu":
            self.final_act = tf.nn.relu
        elif self.conf["final_act"] == "linear":
            self.final_act = tf.identity
        elif self.conf["final_act"] == "leaky_relu":
            self.final_act = tf.nn.leaky_relu
        else:
            raise ValueError("There is not final_act named '%s'." % self.conf["final_act"])

        if self.conf["loss_function"] == "bpr_max":
            self.loss_fun = self._bpr_max_loss
        elif self.conf["loss_function"] == "top1_max":
            self.loss_fun = self._top1_max_loss
        else:
            raise ValueError("There is not loss named '%s'." % self.conf["loss"])
    ...
    def _softmax_neg(self, logits):
        ...
        e_x = e_x / tf.reduce_sum(e_x, axis=1, keepdims=True)
        return e_x  # (b, size_y)
